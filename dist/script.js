...
var forms = function forms(state) {
  ...
  form.forEach(function (item) {
    item.addEventListener('submit', function (e) {
      e.preventDefault();
      var statusMessage = document.createElement('div'); // Creating block with a message status

      statusMessage.classList.add('status');
      item.appendChild(statusMessage);
      var formData = new FormData(item); // Gathering all data from entering form
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  header.addEventListener("click", function (e) {
    ...
    if (target && (target.classList.contains(tabSelector.substring(1)) || target.parentNode.classList.contains(tabSelector.substring(1)))) {
